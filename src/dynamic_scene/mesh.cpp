...
Mesh::Mesh(Collada::PolymeshInfo& polyMesh, const Matrix4x4& transform) {
    ...
    // HACK(kayvonf): move the transform (parsed from the scene json and passed
    // to the constructor by the caller) into the object's position. There's
    // definitely a cleaner way to do this since position must have also been
    ...
    // FIXME(kayvonf): I do not understand why we are copying data from the
    // PolyMesh structure into local variables below (step 1), and then copying
    // the results into the non-indexed buffers (step 2). Step 2 could be
    // completed directly from the PolyMesh structure.
    ...
    // Allocate and populate them here.  They are a non-indexed representation,
    // in that there are three values per polygon.
    ...
    for (int i = 0; i < numTriangles_; ++i) {
        for (int j = 0; j < 3; ++j) {
            positionData_.push_back(
                positions[polyMesh.polygons[i].vertex_indices[j]]);
            normalData_.push_back(
                normals[polyMesh.polygons[i].normal_indices[j]]);
            diffuseColorData_.push_back(diffuseColors[i]);
        }
        if (textureCoordinates.size() > 0) {
            for (int j = 0; j < 3; ++j) {
                texcoordData_.push_back(
                    textureCoordinates[polyMesh.polygons[i]
                                           .texcoord_indices[j]]);
            }
        }
    }
    ...
    //
    // allocate all the OpenGL vertex buffers, and copy the contents of the
    // local buffers into them
    //

    // Sanity check for struct layout in case of unconventional compiler
    static_assert(
        sizeof(Vector3Df) == 3 * sizeof(float),
        "Fatal error: Vector3Df struct has extra padding on this platform.");
    static_assert(
        sizeof(Vector2Df) == 2 * sizeof(float),
        "Fatal error: Vector2Df struct has extra padding on this platform.");

    checkGLError("begin mesh vertex buffer setup");

    positionBufferId_ = gl_mgr_->createVertexBufferFromData(
        (const float*)&positionData_[0], positionData_.size() * 3);

    // printf("Position buffer object: %u\n", positionBufferId.id);

    checkGLError("before normal buffers");

    normalBufferId_ = gl_mgr_->createVertexBufferFromData(
        (const float*)&normalData_[0], normalData_.size() * 3);

    // printf("Normal buffer object: %u\n", normalBufferId.id);

    checkGLError("before texcoord buffers");

    texcoordBufferId_ = gl_mgr_->createVertexBufferFromData(
        (const float*)&texcoordData_[0], texcoordData_.size() * 2);

    // printf("Texcoord buffer object: %u\n", texcoordBufferId.id);

    checkGLError("before tangent buffers");

    tangentBufferId_ = gl_mgr_->createVertexBufferFromData(
        (const float*)&tangentData_[0], tangentData_.size() * 3);

    // printf("Tangent buffer object: %u\n", tangentBufferId.id);

    checkGLError("before diffuse color buffers");

    diffuseColorBufferId_ = gl_mgr_->createVertexBufferFromData(
        (const float*)&diffuseColorData_[0], diffuseColorData_.size() * 3);
    ...
    if (polyMesh.diffuse_filename != "") {
        unsigned int error = lodepng::decode(diffuse_texture,
                                             diffuse_texture_width,
                                             diffuse_texture_height,
                                             polyMesh.diffuse_filename);
        if (error)
            cerr << "Texture (diffuse) loading error = "
                 << polyMesh.diffuse_filename << endl;
        diffuseTextureId_ =
            gl_mgr_->createTextureFromData(diffuse_texture.data(),
                                           diffuse_texture_width,
                                           diffuse_texture_height);
    ...
    if (polyMesh.normal_filename != "") {
        unsigned int error = lodepng::decode(normal_texture,
                                             normal_texture_width,
                                             normal_texture_height,
                                             polyMesh.normal_filename);
        if (error)
            cerr << "Texture (normal) loading error = "
                 << polyMesh.normal_filename << endl;
        normalTextureId_ = gl_mgr_->createTextureFromData(
            normal_texture.data(), normal_texture_width, normal_texture_height);
    ...
    if (polyMesh.environment_filename != "") {
        unsigned int error = lodepng::decode(environment_texture,
                                             environment_texture_width,
                                             environment_texture_height,
                                             polyMesh.environment_filename);
        if (error)
            cerr << "Texture (environment) loading error = "
                 << polyMesh.environment_filename << endl;
        environmentTextureId_ =
            gl_mgr_->createTextureFromData(environment_texture.data(),
                                           environment_texture_width,
                                           environment_texture_height);
...
void Mesh::internalDraw(bool shadowPass, const Matrix4x4& worldToNDC) const {
    ...
    if (shadowPass) {
        ...
        shadowShader->setMatrixParameter("obj2worldNorm",
                                         objectToWorldForNormals);
        ...
        shader_->setScalarParameter("useTextureMapping",
                                    doTextureMapping_ ? 1 : 0);
        shader_->setScalarParameter("useNormalMapping",
                                    doNormalMapping_ ? 1 : 0);
        shader_->setScalarParameter("useEnvironmentMapping",
                                    doEnvironmentMapping_ ? 1 : 0);
        ...
        shader_->setVectorParameter("camera_position",
        ...
        // They should go from object space to the "light space" for each spot
        // light. In this way, the shader can compute the texture coordinate to
        // sample from the Shadow Map given any point on the object. For
        // examples of passing arrays to the shader, look below for
        // "directional_light_vectors[]" etc.

        checkGLError("before binding shadow params");

        for (int j = 0; j < scene_->getNumSpotLights(); j++) {
            string w2lVarName = "world2light[" + std::to_string(j) + "]";
            shader_->setMatrixParameter(w2lVarName,
                                        scene_->getWorldToShadowLight(j));
        }

        checkGLError("after binding shadow params");

        checkGLError(
            "after bind uniforms, about to bind "
            "textures");
        ...
        if (doTextureMapping_)
            shader_->setTextureSampler("diffuseTextureSampler",
                                       diffuseTextureId_);

        // TODO CS248 Part 3: Normal Mapping:
        // You want to pass the normal texture into the
        // shader program. See diffuseTextureSampler for
        // an example of passing textures.

				if (doTextureMapping_)
					shader_->setTextureSampler("normalTextureSampler", normalTextureId_);

        // TODO CS248 Part 4: Environment Mapping:
        // You want to pass the environment texture into
        // the shader program. See diffuseTextureSampler
        // for an example of passing textures.

				if (doEnvironmentMapping_)
					shader_->setTextureSampler("environmentTextureSampler", environmentTextureId_);

        // TODO CS248 Part 5.2: Shadow Mapping:
        // You want to pass the array of shadow textures
        // computed during shadow pass into the shader
        // program. See Scene::visualizeShadowMap for an
        // example of passing texture arrays. See
        // shadow_viz.frag for an example of using
        // texture arrays in the shader.

        // bind light parameters
        // //////////////////////////////////

        shader_->setTextureArraySampler("depthTextureArray",
                                        scene_->getShadowTextureArrayId());

        shader_->setScalarParameter("num_directional_lights",
                                    (int)scene_->getNumDirectionalLights());
        shader_->setScalarParameter("num_point_lights",
                                    (int)scene_->getNumPointLights());
        shader_->setScalarParameter("num_spot_lights",
                                    (int)scene_->getNumSpotLights());

        for (int j = 0; j < scene_->getNumDirectionalLights(); j++) {
            string varname =
                "directional_light_vectors[" + std::to_string(j) + "]";
            const StaticScene::DirectionalLight* light =
                scene_->getDirectionalLight(j);
        ...
        for (int j = 0; j < scene_->getNumSpotLights(); j++) {
            ...
            Vector3D value(
                light->radiance.r, light->radiance.g, light->radiance.b);
            shader_->setVectorParameter(varname, value);
        }

        // bind per-vertex attribute buffers.  These are
        // "in" parameters to the vertex shader
...
void Mesh::reloadShaders() { shader_->reload(); }

BBox Mesh::getBBox() const {
    BBox bbox;

    // convert object-space points to world space points, and compute a
    // world-space bounding box NOTE(kayvonf): this is an inefficient
    // implementation since we're considering each unique vertex position up to
    // 3 times
    for (int i = 0; i < 3 * numTriangles_; ++i) {
        Vector4D posObj(
            positionData_[i].x, positionData_[i].y, positionData_[i].z, 1.f);
