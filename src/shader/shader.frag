...
vec3 Phong_BRDF(vec3 L, vec3 V, vec3 N, vec3 diffuse_color, vec3 specular_color, float specular_exponent)
{
    ...
    return (max(0, dot(Lhat, Nhat)) * diffuse_color) + (pow(max(0, dot(Rhat, Vhat)), specular_exponent) * specular_color);
}
...
vec3 SampleEnvironmentMap(vec3 D)
{
    ...
    vec2 sample = vec2((2 * PI - phi) / (2 * PI), theta / (PI));
    return texture(environmentTextureSampler, sample).rgb;
}
...
void main(void)
{
    ...
    if (useNormalMapping) {
       ...
       // vec3 n = normalize(tan2obj * normal)
    ...
    if (useMirrorBRDF) {
        ...
        //
        vec3 wi = normalize(dir2camera); 
        vec3 R = -wi + 2 * (dot(wi, N)) * N;
        //
